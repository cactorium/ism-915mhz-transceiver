...
# version 0.0.3
# - add -auto flag to automatically include all the files in the current directory
...
def main():
  ...
  parser.add_argument("-a", "--auto", action="store_true", help="automatically include all schematics in the current directory")
  parser.add_argument("input", help="file to autofill")
  parser.add_argument("output", help="file to write autofilled schematic to, use '-' to overwrite input")
  ...
  if args.auto:
    if included_files is None:
      included_files = []
    included_files.extend(filter(lambda p: p.endswith('.sch'), os.listdir(os.getcwd())))
  ...
  if included_files is not None:
    print("searching additional files: {}".format(" ".join(included_files)))
